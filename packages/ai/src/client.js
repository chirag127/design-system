...
function ensureLoaded() {
	...
	loading = new Promise((resolve, reject) => {
		if (typeof document === 'undefined')
			return reject(new Error('Puter client needs a browser (no document).'))
...
function text(resp) {
	...
	return (
		resp?.message?.content ?? resp?.text ?? resp?.content ?? String(resp ?? '')
	)
}
...
export async function chat(messages, opts = {}) {
	...
	const models = opts.model
		? [opts.model, ...MODELS.client.filter((m) => m !== opts.model)]
		: MODELS.client
