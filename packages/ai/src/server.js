...
function resolveKey(opts = {}) {
	...
	if (!key)
		throw new Error(
			'NVIDIA_API_KEY missing. Set env var (GH Secret, server-side only). Never embed a key.',
		)
	return key
}
...
export async function chat(messages, opts = {}) {
	...
	const models = opts.model
		? [opts.model, ...MODELS.server.filter((m) => m !== opts.model)]
		: MODELS.server
	...
	for (const model of models) {
		try {
			...
			if (!res.ok)
				throw new Error(
					`NVIDIA NIM ${model}: ${res.status} ${await res.text()}`,
				)
