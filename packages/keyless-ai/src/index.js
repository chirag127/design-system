...
import { MODELS as KILO_MODELS, chat as kilo } from '@chirag127/keyless-kilo'
import { MODELS as OVH_MODELS, chat as ovh } from '@chirag127/keyless-ovh'
import {
	MODELS as POLLINATIONS_MODELS,
	chat as pollinations,
} from '@chirag127/keyless-pollinations'
...
function resolveOrder(order) {
	...
	const env =
		typeof process !== 'undefined' ? process?.env?.KEYLESS_ORDER : undefined
	if (env)
		return env
			.split(',')
			.map((s) => s.trim())
...
export async function chat(messages, opts = {}) {
	...
	throw new AggregateError(
		errors,
		`All keyless providers failed (${names.join(', ')})`,
	)
}
