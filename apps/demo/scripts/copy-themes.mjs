...
function inlineImports(cssPath, seen = new Set()) {
  ...
  css = css.replace(/@import\s+['"]([^'"]+)['"]\s*;/g, (_m, spec) => {
    const target = spec.startsWith('.') ? resolve(dirname(abs), spec) : resolveFile(spec, dirname(abs))
...
for (const t of themes) {
  const p = require.resolve(`@chirag127/theme/src/${t}`)
...
writeFileSync(
  ...
  readFileSync(require.resolve('@chirag127/theme/src/theme.js'), 'utf8'),
  'utf8',
)
